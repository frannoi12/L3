...
public class JobViewController {
	...
	public String viewJobs(Model model) {
        model.addAttribute("jobs", jobService.getJobsFromApi());
    ...
    public String addJob(@ModelAttribute Job job) {
        jobService.createAJob(job); // Sauvegarde le job
        return "redirect:/jobsview"; // Redirige vers la liste des jobs
    }
	
	@GetMapping("/detail/{id}")
	public String viewJobDetail(@PathVariable String id, Model model) {
	    model.addAttribute("job", jobService.getJobByIdFromApi(id));
	...
	public String updateJobForm(@PathVariable String id, Model model) {
	    model.addAttribute("job", jobService.getJobByIdFromApi(id)); // Charger les données existantes
	...
	public String updateJob(@PathVariable String id, @ModelAttribute Job job) {
	    jobService.updateApiJob(id, job); // Appeler le service pour la mise à jour
	    return "redirect:/jobsview"; // Rediriger vers la liste des jobs
	}

	@PostMapping("/delete/{id}")
	public String deleteJob(@PathVariable String id) {
	    jobService.deleteApiJob(id); // Supprimer le job via le service
	    return "redirect:/jobsview"; // Rediriger vers la liste des jobs
	}
