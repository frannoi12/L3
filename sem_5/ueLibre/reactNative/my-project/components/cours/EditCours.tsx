...
const UpdateCours = ({navigation,route}) => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Une erreur est survenue lors de la modifications des données.');
      }

      const data = await response.json();
      console.log('Données Modifié:', data);
    ...
    <div className="container">
      <div className="row">
        <div className="col-md-6 offset-md-3 pt-3">
          <h1 className="text-center">Modifier un cours</h1>
          <Formik
